        ...
        NSURLSessionDownloadTask *task = [session
                            downloadTaskWithRequest:request
                              completionHandler:^(NSURL *localFile, NSURLResponse *response,NSError *error) {
                                    if (error) {NSLog(@"Flickr background fetch failed: %@", error.localizedDescription);
...
- (void)setPhotoDatabaseContext:(NSManagedObjectContext *)photoDatabaseContext
{
    ...
    NSDictionary *userInfo = self.photoDatabaseContext ? @{ PhotoDatabaseAvailabilityContext :
                                                                 self.photoDatabaseContext } : nil;
       ...
       // dispatch_once обеспечивает однократный запуск блока (синглтон)
        ...
        dispatch_once(&onceToken, ^{
           ...
           // nil для delegateQueue означает "случайная, не main queue очередь"
            _flickrDownloadSession = [NSURLSession
                                      sessionWithConfiguration:urlSessionConfig
                                                      delegate:self
                                                 delegateQueue:nil];
        });
    }
